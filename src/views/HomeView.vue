...
export default {
  ...
  methods: {
    ...
    suitable(idx, conv, suit) {
      ...
      this.axios
        .put(`/api/ai/suitable/${this.cid}`, cdate)
        .then((result) => {
          console.log(result);
    ...
    send() {
      ...
      var that = this;
      var source = (this.source = new EventSource(
        `/api/chat/${this.cid}?prompt=${encodeURIComponent(chatMsg)}`
      ));
    ...
    generateConvTitle(conv) {
      var that = this;
      var tsource = (this.tsource = new EventSource(
        `/api/chat/title/${this.cid}`
      ));
