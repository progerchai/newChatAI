...
function showSlideMethod() {
  ...
  (slideNavContainer.value as any)?.appendChild(navEle.value);
}
...
function initConvs(convs: Array<{ speaker: string, idx: number, speeches: any[] }>) {
...
function suitable(idx: number, conv: { idx: number, suitable: any }, suit: any) {
  const { cid } = state
  var cdate = {
    idx: idx,
    msg_idx: conv.idx,
...
<template>
  <div id="__next">
                      ...
                      <div
                        ...
                        <div class="relative flex w-[calc(100%-50px)] flex-col gap-1 md:gap-3 lg:w-[calc(100%-115px)]">
                          ...
                          <div class="flex justify-between">
                            <div
                              class="text-gray-400 flex self-end lg:self-center justify-center mt-2 gap-3 md:gap-4 lg:gap-1 lg:absolute lg:top-0 lg:translate-x-full lg:right-0 lg:mt-0 lg:pl-2 visible">
                              <button @click.stop="suitable(idx, conv, 1)" v-if="conv.suitable[conv.idx] == 0 ||
    ...
    " class="p-1 rounded-md hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-200 disabled:dark:hover:text-gray-400">
                  ...
                  <div v-if="state.conversation.length == 0"
                ...
                <transition name="el-fade-in-linear">
                  <!-- 回到底部 -->
                  <button v-show="state.isShowGoBottom" @click="handleScrollBottom"
          ...
          <div
            ...
            <form class="stretch mx-2 flex flex-row gap-3 pt-2 last:mb-2 md:last:mb-6 lg:mx-auto lg:max-w-3xl lg:pt-6">
              <div class="relative flex h-full flex-1 md:flex-col">
                <div class="flex ml-1 md:w-full md:m-auto md:mb-2 gap-0 md:gap-2 justify-center">
                  <button v-if="!state.convLoading && state.conversation.length > 0" @click.stop.prevent="chatRepeat"
                  ...
                  <button v-if="state.convLoading" @click.stop.prevent="stopChat" id="stopChat"
                ...
                <div
                  ...
                  <textarea v-model="state.chatMsg" ref="inputChatRef" @keydown="judgeInput" tabindex="0" data-id="root"
                  ...
                  <button @click.stop.prevent="send" :disabled="state.convLoading"
                    ...
                    <div v-if="state.convLoading" class="text-2xl" style="line-height: 1.3rem">
            ...
            <div class="px-3 pt-2 pb-3 text-center text-xs text-black/50 dark:text-white/50 md:px-4 md:pt-3 md:pb-6">
              <a href="https://gitee.com/MIEAPP/chatai-vue" target="_blank" rel="noreferrer"
                class="underline">开源：chatai-vue</a>
              本开源项目前端使用vue，后端基于openai开放api开发，仅供学习 AI
              使用。
            </div>
          </div>
        </main>
                ...
                <div class="flex flex-col gap-2 text-gray-100 text-sm">
                  <template v-for="(conversation, cidx) in state.conversations">
                    <div v-if="conversation.editable"
                      ...
                      <input id="titleInput" v-model="state.convTitletmp" @blur="titleInputBlur(cidx, conversation)"
    ...
    " class="flex py-3 px-3 items-center gap-3 relative rounded-md cursor-pointer break-all group">
                      ...
                      <div class="flex-1 text-ellipsis max-h-5 overflow-hidden break-all relative">
                        ...
                        <div :class="{
                          ...
                          " class="absolute inset-y-0 right-0 w-8 z-10 bg-gradient-to-l"></div>
                      </div>
              ...
              <a v-if="state.conversations.length > 0" @click.stop.prevent="clearConversations"
              ...
              <a v-if="state.theme == 'light'" @click="changeTheme('dark')"
                class="flex py-3 px-3 items-center gap-3 rounded-md hover:bg-gray-500/10 transition-colors duration-200 text-white cursor-pointer text-sm">
                <svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round"
                  stroke-linejoin="round" class="h-4 w-4" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
              ...
              <a v-if="state.theme == 'dark'" @click="changeTheme('light')"
                class="flex py-3 px-3 items-center gap-3 rounded-md hover:bg-gray-500/10 transition-colors duration-200 text-white cursor-pointer text-sm">
                <svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round"
                  stroke-linejoin="round" class="h-4 w-4" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
                  <circle cx="12" cy="12" r="5"></circle>
  ...
  <div v-show="state.showSlide" class="semi-portal" style="z-index: 1000">
    <div class="">
                ...
                <div class="fixed inset-0 z-40 flex">
                  <div class="relative flex w-full max-w-xs flex-1 flex-col bg-gray-900 translate-x-0"
                    ...
                    <div ref="slideNavContainer" style="width: 320px"
                      class="flex h-full flex-1 items-start border-white/20"></div>
                  </div>
  ...
  <!-- 弹窗 -->
  <div id="headlessui-portal-root" v-if="state.popupShow">
            ...
            <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
              <div v-if="state.popupShow"
                ...
                <div class="prose dark:prose-invert">
                  ...
                  <h4 class="mb-4">
                    本开源项目基于openai开放api开发，使用最新的gpt-3.5-turbo，前端使用vue高仿ChatGpt界面，仅供学习使用.
                  </h4>
                  <div class="flex gap-4 flex-col text-sm">
                    ...
                    <div class="flex p-4 bg-gray-50 dark:bg-white/5 rounded-md items-center gap-4 min-h-[71px]">
                      <div class="w-10 text-2xl text-center">🔬</div>
                      <div class="flex-1 leading-5">
                        随着使用人数的增多和api被墙等原因，演示环境过几天就会关闭问答功能。如果需要长期使用，可以联系我，微信：zjb592466695
                      </div>
                    </div>
                  </div>
                  <div class="flex gap-4 mt-6">
                    <button @click="closePopup" class="btn flex justify-center gap-2 btn-primary ml-auto">
                      Done
                    </button>
                  </div>
                </div>
                ...
                <div class="flex items-center sm:flex">
                  ...
                  <div class="mt-3 text-center sm:mt-0 sm:text-left">
                    <h3 class="text-lg font-medium leading-6 text-gray-900 dark:text-gray-200"
                      id="headlessui-dialog-title-:r5:" data-headlessui-state="open">
